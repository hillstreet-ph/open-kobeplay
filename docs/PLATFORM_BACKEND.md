...
| PostgreSQL schema | `open_kobeplay` |
| Sentry organization | `hillstreet` |
| Sentry project | `open-kobeplay` |
| GitHub repository | `hillstreet-ph/open-kobeplay` |
| Container image | `${DOCKERHUB_USERNAME}/open-kobeplay` |
...
7. Supabase migrations must target schema `open_kobeplay` and must be reviewed before production execution.
